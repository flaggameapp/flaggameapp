...
const fallbackTranslations = {
  ...
  "pt-BR": {
    ...
    worldChallengeContinueInsufficient: "Voce precisa de {cost} moedas para continuar. Saldo: {balance}.",
    worldChallengeContinueFailed: "Nao foi possivel concluir a continuacao. Nenhuma moeda foi gasta.",
    worldChallengeCoinsEarned: "Moedas recebidas",
    worldChallengeCoinBalance: "Saldo",
    worldChallengeContinuationUsed: "Continuacao",
    worldChallengeRankingEligibility: "Ranking futuro",
    worldChallengeRankingEligible: "Elegivel",
    worldChallengeRankingNotEligible: "Nao elegivel",
    worldChallengeRankingUnavailable: "Indisponivel",
    worldChallengeYes: "Sim",
    worldChallengeNo: "Nao",
    ...
    playGamesStatusUnavailable: "Google Play Games indisponivel",
    playGamesStatusPending: "Sincronizacao do Play Games pendente",
...
async function fetchTranslations(language) {
  ...
  for (const url of getTranslationUrls(language)) {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Nao foi possivel carregar ${url}: ${response.status}`
        );
      }
  ...
  throw lastError || new Error("Nao foi possivel carregar o idioma.");
}
